...
const user = await currentUser()
  ...
  return (
    ...
    <ul className="flex flex-col gap-2 h-screen">
      {conversation.messages.slice(1).map((message: Message) => (message.role === "user" ? <UserMessage userImageUrl={user.imageUrl}  key={message.id} message={message} /> : 
...
function SystemMessage({message}: {message: Message}) {
  return (
    <li className="flex items-start gap-4">
      <Image className="rounded-full object-cover" src="/tree.png" alt="system avatar" height={32} width={32}/>
...
function SystemMessageActions() {
  return (
    <ul className="flex gap-2 opacity-0 hover:opacity-100 cursor-pointer items-center">
      <MessageActionItem >
        <Image src="/copy.svg" width={16} height={16} alt="copy answer to clipboard button" />
      </MessageActionItem>
      <MessageActionItem>
        <Image src="/like.svg" width={16} height={16} alt="like answer" />
      </MessageActionItem>
        <MessageActionItem>
          <Image src="/downvote.svg" width={16} height={16} alt="downvote answer" />
        </MessageActionItem>
    </ul>
  )
}
...
function UserMessage({message,userImageUrl}: {message: Message, userImageUrl: string}) {
  return (
    <li className="flex items-start gap-4">
        <Image className="rounded-full object-cover" src={userImageUrl} alt="user avatar" height={32} width={32}/>
      <div className="flex flex-col">
